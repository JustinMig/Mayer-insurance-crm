...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (honey) {
      // Return success so automated spam bots do not learn
    ...
    const firstName = field(payload, [
      'firstName',
      'first_name',
      'First Name',
    ])

    const lastName = field(payload, [
      'lastName',
      'last_name',
      'Last Name',
    ])

    const addressLine1 = field(payload, [
      'address',
      'addressLine1',
      'Street Address',
    ])

    const city = field(payload, [
      'city',
      'City',
    ])

    const state = field(payload, [
      'state',
      'State',
    ])

    const zipCode = field(payload, [
      'zip',
      'zipCode',
      'ZIP Code',
    ])

    const phone = field(payload, [
      'phone',
      'phoneNumber',
      'Phone Number',
    ])

    const email = field(payload, [
      'email',
      'emailAddress',
      'Email Address',
    ])
     ...
     * Prevent the same phone/email from generating another
     * website lead within two minutes.
     */
    const twoMinutesAgo = new Date(
      Date.now() - 2 * 60 * 1000
    ).toISOString()

    const { data: recentLeads, error: duplicateCheckError } =
      await supabase
        .from('website_leads')
    ...
    if (duplicateCheckError) {
      console.error(
        'Website lead duplicate check failed',
        duplicateCheckError
      )
    }
