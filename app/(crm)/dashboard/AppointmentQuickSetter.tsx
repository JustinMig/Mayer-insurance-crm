...
export default function AppointmentQuickSetter() {
  ...
  const [appointmentAgents, setAppointmentAgents] = useState<AppointmentAgent[]>([])
  const [coordinator, setCoordinator] = useState(false)
  const [contextLoading, setContextLoading] = useState(true)
  const [status, setStatus] = useState('Loading appointment calendar…')
  ...
  useEffect(() => {
    ...
    void fetch('/api/workspace/calendar-context', { cache: 'no-store' })
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(result.error || 'Unable to load appointment calendar.')
        if (cancelled) return
        const agents = Array.isArray(result.agents) ? result.agents as AppointmentAgent[] : []
        const isCoordinator = Boolean(result.coordinator)
        setAppointmentAgents(agents)
  ...
  useEffect(() => {
    ...
    if (coordinator && calendarOwnerId) {
      const selected = appointmentAgents.find((agent) => agent.id === calendarOwnerId)
      setStatus(`Scheduling for ${selected?.full_name || 'selected agent'}. Choose a client and date.`)
    }
  }, [calendarOwnerId, coordinator, appointmentAgents])
  ...
  useEffect(() => {
    setBlocks([])
    setStartTime('')
    if (!eventDate || !calendarOwnerId) return

    let cancelled = false
    setChecking(true)
    setStatus('Checking the selected agent calendar for booked times…')
    const params = new URLSearchParams({ date: eventDate, owner: calendarOwnerId })
  ...
  async function saveAppointment() {
    ...
    setSaving(true)
    setStatus('Saving appointment to the selected agent calendar…')
    try {
      ...
      const response = await fetch('/api/workspace/events', {
        ...
        body: JSON.stringify({
          assigned_agent_id: calendarOwnerId,
  ...
  return (
    <div className="quick-appointment-setter">
      ...
      <div className="quick-appointment-mode" role="group" aria-label="Appointment person type">
        <button type="button" className={mode === 'existing' ? 'active' : ''} onClick={() => switchMode('existing')} disabled={!calendarOwnerId}>EXISTING CLIENT</button>
        <button type="button" className={mode === 'new' ? 'active' : ''} onClick={() => switchMode('new')} disabled={!calendarOwnerId}>NEW / NON-CLIENT</button>
      </div>
      ...
      {mode === 'existing' ? (
        <>
          <label className="label quick-appointment-client">
            ...
            <input
              ...
              placeholder={calendarOwnerId ? 'Type client name or phone' : 'Choose an agent first'}
              autoComplete="off"
        ...
        <div className="quick-appointment-grid quick-appointment-new-person">
          <label className="label">
            ...
            <input className="input" value={newName} onChange={(event) => setNewName(event.target.value)} placeholder="First and last name" autoComplete="off" disabled={!calendarOwnerId} />
          </label>
          <label className="label">
            <span>Phone number</span>
            <input className="input" inputMode="tel" value={newPhone} onChange={(event) => setNewPhone(formatPhoneInput(event.target.value))} placeholder="(555) 555-5555" autoComplete="tel" disabled={!calendarOwnerId} />
          </label>
        </div>
      )}
      ...
      <div className="quick-appointment-grid">
        <label className="label">
          ...
          <input className="input" type="date" value={eventDate} onChange={(event) => setEventDate(event.target.value)} disabled={!calendarOwnerId} />
        </label>
        <label className="label">
          <span>Appointment time</span>
          <select className="select" value={startTime} onChange={(event) => setStartTime(event.target.value)} disabled={!calendarOwnerId || !eventDate || checking}>
      ...
      <label className="label">
        ...
        <textarea className="textarea" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Purpose of appointment or anything to remember" disabled={!calendarOwnerId} />
      </label>
      ...
      <style jsx global>{`
        ...
        .quick-appointment-agent{padding:12px;border:1px solid #d8d0ef;border-radius:11px;background:#f6f3ff}.quick-appointment-agent .select{min-height:44px;background:#fff}
        .quick-appointment-mode{display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:4px;border:1px solid #d6e0e8;border-radius:11px;background:#eef3f6}.quick-appointment-mode button{min-height:40px;border:0;border-radius:8px;background:transparent;color:#627386;font:inherit;font-size:.75rem;font-weight:900;cursor:pointer}.quick-appointment-mode button.active{background:#fff;color:#2f5274;box-shadow:0 1px 4px rgba(15,23,42,.1)}.quick-appointment-mode button:disabled{opacity:.5;cursor:default}
