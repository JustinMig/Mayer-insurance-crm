...
export default function DashboardCalendar({ agents, viewerName }: { agents: Agent[]; viewerName: string }) {
  ...
  function queueCard(event: CalendarEvent, mode: 'today' | 'reschedule') {
    ...
    return (
      <article className={`dash-cal-view-card ${tone}`} key={event.id}>
        <div className="dash-cal-view-pills">
          ...
          <span className="dash-cal-type">Appointment</span>
  ...
  return (
    <section className="dashboard-calendar-block">
      <div className="dashboard-calendar-queue-buttons">
        <button type="button" className="dashboard-today-button" onClick={() => setViewMode('today')}>
          TODAY&apos;S APPOINTMENTS <span>{todayAppointments.length}</span>
        </button>
        <button type="button" className="dashboard-reschedule-button" onClick={() => setViewMode('reschedule')}>
          RESCHEDULE <span>{rescheduledAppointments.length}</span>
        </button>
      </div>
      ...
      <div className="card dashboard-calendar-card">
        ...
        {error ? <div className="notice dashboard-calendar-error">{error}</div> : null}
              ...
              <button type="button" className={`dashboard-calendar-day${outside ? ' outside' : ''}${isToday ? ' today' : ''}`} key={key} onClick={() => openDay(key)}>
                ...
                <div className="dashboard-calendar-day-events">
                  {items.slice(0, 3).map((event) => {
                    const agent = agentById.get(event.assigned_agent_id)
                    return (
                      <div className={`dashboard-calendar-event ${agentTone(agent?.full_name || '')}`} key={event.id}>
      ...
      {viewMode ? (
        <div className="dash-cal-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) closeView() }}>
          <section className="dash-cal-modal" role="dialog" aria-modal="true">
            <div className="dash-cal-modal-head">
              <div>
                <h2>{viewMode === 'today' ? 'Today’s Appointments' : viewMode === 'reschedule' ? 'Reschedule' : selectedDay ? formatDate(selectedDay) : 'Calendar'}</h2>
                {viewMode === 'today' ? <p className="subtle">{formatDate(todayKey)}</p> : null}
              </div>
              <button type="button" className="btn btn-secondary btn-small" onClick={closeView}>Close</button>
            </div>

            {viewMode === 'today' ? (
              <div className="dash-cal-view-list">
                {!todayAppointments.length ? <div className="card card-pad empty">No active appointments for today.</div> : todayAppointments.map((event) => queueCard(event, 'today'))}
              </div>
            ) : null}

            {viewMode === 'reschedule' ? (
              <div className="dash-cal-view-list">
                {!rescheduledAppointments.length ? <div className="card card-pad empty">No appointments waiting to be rescheduled.</div> : rescheduledAppointments.map((event) => queueCard(event, 'reschedule'))}
              </div>
            ) : null}

            {viewMode === 'day' ? (
              <div className="dash-cal-view-list">
                {!dayItems.length ? <div className="card card-pad empty">Nothing scheduled for this day.</div> : dayItems.map((event) => {
                  const agent = agentById.get(event.assigned_agent_id)
                  const linked = event.client_id ? clientById.get(event.client_id) : undefined
                  const tone = agentTone(agent?.full_name || '')
                  return (
                    <article className={`dash-cal-view-card ${tone}`} key={event.id}>
                      <div className="dash-cal-view-pills"><span className={`dash-cal-agent ${tone}`}>{agent?.full_name.split(' ')[0] || viewerName.split(' ')[0] || 'Agent'}</span><span className="dash-cal-type">{event.event_type === 'appointment' ? 'Appointment' : 'Activity'}</span></div>
                      <h3>{event.title}</h3>
                      <strong className="dash-cal-view-time">{formatTime(event.start_time)}{event.end_time ? ` – ${formatTime(event.end_time)}` : ''}</strong>
                      {linked ? <div className="dash-cal-client"><span>Client</span><strong>{clientName(linked)}</strong>{linked.phone ? <small>{linked.phone}</small> : null}</div> : null}
                      {event.notes ? <div className="dash-cal-notes"><strong>Notes</strong><p>{event.notes}</p></div> : null}
                      {linked ? <a className="btn btn-secondary btn-small" href={`/clients/${linked.id}`}>OPEN CLIENT</a> : null}
                    </article>
      ...
      <style>{`
        .dashboard-calendar-block{margin-top:20px}.dashboard-calendar-queue-buttons{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px}.dashboard-today-button,.dashboard-reschedule-button{border:0;border-radius:13px;padding:15px 18px;font-weight:900;letter-spacing:.02em;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;min-height:52px}.dashboard-today-button{background:#2563eb;color:#fff}.dashboard-reschedule-button{background:#cbd5e1;color:#172033}.dashboard-today-button span,.dashboard-reschedule-button span{display:inline-grid;place-items:center;min-width:24px;height:24px;padding:0 7px;border-radius:999px;background:rgba(255,255,255,.22);font-size:.78rem}.dashboard-reschedule-button span{background:rgba(255,255,255,.65)}.dashboard-calendar-card{padding:16px;overflow:hidden}.dashboard-calendar-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap}.dashboard-calendar-head h2{margin:0}.dashboard-calendar-legend{display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-top:6px}.dashboard-calendar-legend span{display:flex;align-items:center;gap:5px;font-size:.82rem;font-weight:800}.dashboard-calendar-legend i,.dashboard-calendar-mobile-dots i{width:10px;height:10px;border-radius:999px;background:#64748b}.dashboard-calendar-legend i.justin,.dashboard-calendar-mobile-dots i.justin{background:#2563eb}.dashboard-calendar-legend i.isaiah,.dashboard-calendar-mobile-dots i.isaiah{background:#dc2626}.dashboard-calendar-controls{display:flex;gap:6px}.dashboard-calendar-month{text-align:center;font-weight:900;font-size:1.08rem;margin:12px 0}.dashboard-calendar-loading{margin-bottom:8px}.dashboard-calendar-error{margin-bottom:10px}.dashboard-calendar-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));border:1px solid #dce4ea;border-radius:12px;overflow:hidden}.dashboard-calendar-weekday{padding:8px 4px;text-align:center;font-size:.74rem;font-weight:900;color:#64748b;background:#f8fafc;border-bottom:1px solid #dce4ea}.dashboard-calendar-day{appearance:none;border:0;border-right:1px solid #e5eaf0;border-bottom:1px solid #e5eaf0;background:#fff;min-width:0;min-height:112px;padding:6px;text-align:left;position:relative;cursor:pointer;color:#172033}.dashboard-calendar-day:nth-child(7n){border-right:0}.dashboard-calendar-day.outside{background:#f8fafc;color:#94a3b8}.dashboard-calendar-day.today{box-shadow:inset 0 0 0 2px #10263f}.dashboard-calendar-day-number{font-weight:900}.dashboard-calendar-day-events{display:grid;gap:4px;margin-top:5px}.dashboard-calendar-event{display:grid;gap:1px;border-radius:6px;padding:4px 5px;font-size:.68rem;background:#eef2f6;border-left:3px solid #64748b;overflow:hidden}.dashboard-calendar-event.justin{background:#dbeafe;color:#1e3a8a;border-left-color:#2563eb}.dashboard-calendar-event.isaiah{background:#fee2e2;color:#7f1d1d;border-left-color:#dc2626}.dashboard-calendar-event span{font-size:.62rem}.dashboard-calendar-event strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.dashboard-calendar-day-events small{font-size:.65rem;font-weight:800;color:#64748b}.dashboard-calendar-mobile-dots{display:none}.dash-cal-modal-backdrop{position:fixed;inset:0;z-index:1600;background:rgba(15,23,42,.58);display:grid;place-items:center;padding:14px}.dash-cal-modal{width:min(820px,100%);max-height:calc(100dvh - 28px);overflow:auto;background:#fff;border-radius:18px;box-shadow:0 24px 80px rgba(15,23,42,.3);padding:18px}.dash-cal-modal-head{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:16px}.dash-cal-modal-head h2{margin:0}.dash-cal-modal-head p{margin:5px 0 0}.dash-cal-view-list{display:grid;gap:12px}.dash-cal-view-card{border:1px solid #dce4ea;border-left:5px solid #64748b;border-radius:13px;padding:14px;display:grid;gap:10px}.dash-cal-view-card.justin{border-left-color:#2563eb}.dash-cal-view-card.isaiah{border-left-color:#dc2626}.dash-cal-view-card h3{margin:0}.dash-cal-view-pills{display:flex;gap:6px;flex-wrap:wrap}.dash-cal-agent,.dash-cal-type,.dash-cal-reschedule-pill{font-size:.72rem;font-weight:900;border-radius:999px;padding:4px 8px;background:#eef2f6;color:#334155}.dash-cal-agent.justin{background:#dbeafe;color:#1d4ed8}.dash-cal-agent.isaiah{background:#fee2e2;color:#b91c1c}.dash-cal-reschedule-pill{background:#ffedd5;color:#9a3412}.dash-cal-view-time{color:#475569}.dash-cal-client{display:flex;gap:8px;align-items:center;flex-wrap:wrap;border-radius:10px;background:#eff6ff;padding:9px 11px}.dash-cal-client span{font-size:.72rem;font-weight:900;color:#64748b;text-transform:uppercase}.dash-cal-client small{color:#64748b}.dash-cal-notes,.dash-cal-reschedule-note{border-radius:10px;background:#f8fafc;padding:10px 11px}.dash-cal-reschedule-note{background:#fff7ed}.dash-cal-notes p,.dash-cal-reschedule-note p{margin:5px 0 0;white-space:pre-wrap}
        @media(max-width:720px){.dashboard-calendar-block{margin-top:14px}.dashboard-calendar-queue-buttons{gap:8px}.dashboard-today-button,.dashboard-reschedule-button{padding:11px 8px;min-height:46px;font-size:.72rem}.dashboard-calendar-card{padding:10px}.dashboard-calendar-head{align-items:center}.dashboard-calendar-legend{gap:9px}.dashboard-calendar-month{margin:9px 0}.dashboard-calendar-weekday{padding:7px 1px;font-size:.62rem}.dashboard-calendar-day{min-height:62px;padding:4px 3px;text-align:center}.dashboard-calendar-day-number{font-size:.78rem}.dashboard-calendar-day-events{display:none}.dashboard-calendar-mobile-dots{display:flex;justify-content:center;gap:3px;flex-wrap:wrap;margin-top:5px}.dashboard-calendar-mobile-dots i{width:6px;height:6px}.dash-cal-modal{padding:14px}.dash-cal-view-card{padding:12px}}
        @media(max-width:380px){.dashboard-today-button,.dashboard-reschedule-button{font-size:.66rem;letter-spacing:0}.dashboard-today-button span,.dashboard-reschedule-button span{min-width:20px;height:20px;padding:0 5px}.dashboard-calendar-weekday{font-size:.57rem}.dashboard-calendar-day{min-height:58px}.dashboard-calendar-controls .btn{padding-inline:8px}}
      `}</style>
    </section>
  )
}
