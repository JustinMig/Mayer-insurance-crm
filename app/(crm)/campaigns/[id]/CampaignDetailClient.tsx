...
type AppointmentState = { row: CampaignRow } | null
type RecordOptions = { note?: string; nextAction?: string; followUpDate?: string; followUpTime?: string }
...
function rowPriority(row: CampaignRow, today: string) {
...
type ClientCardProps = {
  ...
  isBusy: boolean
  today: string
  onOpenConversation: (row: CampaignRow) => void
  onRecord: (row: CampaignRow, outcome: string) => void
  onUndo: (row: CampaignRow) => void
  onAppointment: (row: CampaignRow) => void
  onRemove: (row: CampaignRow) => void
}
  ...
  return (
    <article className={`campaign-client-row${focus ? ' campaign-client-row-focus' : ''}`}>
      ...
      <div className="campaign-action-cell">
        <button className="campaign-primary-action" type="button" disabled={isBusy} onClick={() => onOpenConversation(row)}>SPOKE / UPDATE</button>
        <div className="campaign-attempt-actions">
          <button type="button" disabled={isBusy} onClick={() => onRecord(row, 'no_answer')}>No answer</button>
          <button type="button" disabled={isBusy} onClick={() => onRecord(row, 'voicemail')}>Voicemail</button>
...
export default function CampaignDetailClient({ campaign, initialRows, agents, viewerId, canViewAll }: {
  ...
  }, [filter, ownerFilter, workMode])

  const ownerRows = useMemo(
    () => rows.filter((row) => ownerFilter === 'all' || row.assigned_agent_id === ownerFilter),
    [rows, ownerFilter]
  )

  const sortedRows = useMemo(() => [...ownerRows].sort((a, b) => {
    const difference = rowPriority(a, today) - rowPriority(b, today)
    if (difference) return difference
    if (a.status === 'attempted' && b.status === 'attempted') {
      return String(a.last_contacted_at || '').localeCompare(String(b.last_contacted_at || ''))
    }
    if (a.follow_up_date || b.follow_up_date) {
      return String(a.follow_up_date || '9999-12-31').localeCompare(String(b.follow_up_date || '9999-12-31'))
    }
  ...
  const stats = useMemo(() => {
    ...
    for (const row of ownerRows) {
      if (row.status === 'not_contacted') summary.notContacted += 1
      else if (row.status === 'attempted') summary.attempted += 1
      else if (row.status === 'spoke') summary.spoke += 1
      else if (row.status === 'follow_up') summary.followUp += 1
      else if (row.status === 'completed') summary.completed += 1
      if (RESOLVED.has(row.status)) summary.resolved += 1
    }
    return summary
  }, [ownerRows])
  ...
  const displayRows = useMemo(() => visibleRows.slice(0, visibleLimit), [visibleRows, visibleLimit])
  ...
  const postCampaign = useCallback(async (body: Record<string, unknown>) => {
  ...
  const postMemberAction = useCallback(async (body: Record<string, unknown>) => {
  ...
  const openConversation = useCallback((row: CampaignRow) => {
    setAppointment(null)
    setDialog({ row })
    setResult('spoke')
    setNote('')
  ...
  const record = useCallback(async (row: CampaignRow, outcome: string, options: RecordOptions = {}) => {
    if (!startBusy(row.id)) return
    setMessage('')
    try {
  ...
  }, [finishBusy, postCampaign, startBusy])

  const undoLast = useCallback(async (row: CampaignRow) => {
    if (!window.confirm(
      `Undo the last Outreach result for ${clientName(row.client)}?\n\n` +
      'This removes only the most recent status/result so you can choose the correct one. ' +
      'If that result created a follow-up calendar item, that item will also be removed.'
    )) return
    if (!startBusy(row.id)) return
    setMessage('')
    try {
      const data = await postMemberAction({ action: 'undo_last', member_id: row.id })
      const updated = data.member || {}
      setRows((current) => current.map((item) => item.id === row.id ? {
        ...item,
        status: updated.status || 'not_contacted',
        last_outcome: updated.last_outcome ?? null,
        last_note: updated.last_note ?? null,
        last_contacted_at: updated.last_contacted_at ?? null,
        next_action: updated.next_action ?? null,
        follow_up_date: updated.follow_up_date ?? null,
        follow_up_time: updated.follow_up_time ?? null,
        attempt_count: Number(updated.attempt_count ?? Math.max(Number(item.attempt_count || 0) - 1, 0))
      } : item))
      setMessage(`${clientName(row.client)} restored to ${statusLabel(updated.status || 'not_contacted')}.`)
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Unable to undo the last Outreach result.')
    } finally {
      finishBusy()
    }
  }, [finishBusy, postMemberAction, startBusy])

  const openAppointment = useCallback((row: CampaignRow) => {
    setDialog(null)
    setAppointment({ row })
  ...
  const removeRow = useCallback(async (row: CampaignRow) => {
    ...
    setMessage('')
    try {
      await postCampaign({ action: 'remove_member', member_id: row.id })
  ...
  async function saveAppointment() {
    ...
    if (!startBusy(appointment.row.id)) return
  ...
  const cards = useMemo(() => displayRows.map((row) => (
    <ClientCard
      key={row.id}
      row={row}
      ...
      onRemove={removeRow}
    />
  )), [busyId, canViewAll, displayRows, openAppointment, openConversation, record, removeRow, today, undoLast])

  const nextRow = readyRows[0] || null
  const nextCard = useMemo(() => nextRow ? (
    <ClientCard
      row={nextRow}
      focus
      canViewAll={canViewAll}
      isBusy={busyId === nextRow.id}
      today={today}
      onOpenConversation={openConversation}
      onRecord={record}
      onUndo={undoLast}
      ...
      onRemove={removeRow}
    />
  ) : null, [busyId, canViewAll, nextRow, openAppointment, openConversation, record, removeRow, today, undoLast])

  const completionPercent = stats.total ? Math.round((stats.resolved / stats.total) * 100) : 0
  const remainingRows = Math.max(visibleRows.length - displayRows.length, 0)
  ...
  return (
    <>
          ...
          <>
            ...
            {!visibleRows.length ? (
              ...
              <section className="campaign-client-table">
                ...
                {cards}
                {remainingRows ? (
                  <div className="campaign-load-more-row">
                    <span>Showing {displayRows.length} of {visibleRows.length}</span>
                    <button type="button" onClick={() => setVisibleLimit((current) => current + PAGE_SIZE)}>
                      LOAD {Math.min(PAGE_SIZE, remainingRows)} MORE
                    </button>
                  </div>
                ) : null}
              </section>
      ...
      {appointment ? (
        <div className="outreach-dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busyId) setAppointment(null) }}>
          <div className="outreach-dialog" role="dialog" aria-modal="true" aria-label="Create client appointment">
            ...
            {message ? <div className="notice" style={{ marginTop: 10 }}>{message}</div> : null}
            <div className="outreach-dialog-actions"><button type="button" className="campaign-quiet-button" disabled={Boolean(busyId)} onClick={() => setAppointment(null)}>Cancel</button><button type="button" className="campaign-work-button" disabled={Boolean(busyId)} onClick={() => void saveAppointment()}>{busyId ? 'Saving…' : 'Add to calendar'}</button></div>
          </div>
        </div>
      ) : null}
      ...
      <style jsx global>{`
        ...
        @media(max-width:900px){.campaign-table-heading{display:none}.campaign-client-table{border-radius:9px}.campaign-client-row{grid-template-columns:1fr 1fr;gap:10px 18px;contain-intrinsic-size:220px}.campaign-person-cell{grid-column:1}.campaign-status-cell{grid-column:2;align-items:flex-end}.campaign-status-meta{align-items:flex-end}.campaign-activity-cell{grid-column:1 / -1;padding-top:8px;border-top:1px solid #edf0f2}.campaign-action-cell{grid-column:1 / -1;flex-direction:row;align-items:center;flex-wrap:wrap}.campaign-primary-action{width:auto;min-width:145px}.campaign-control-strip{border-radius:9px}}
        @media(max-width:680px){.campaign-detail-titlebar{align-items:flex-start;flex-direction:column}.campaign-heading-actions{width:100%}.campaign-heading-actions>*{flex:1}.campaign-control-strip{display:grid;grid-template-columns:repeat(3,1fr)}.campaign-progress-compact{grid-column:1 / -1}.campaign-stat-inline{min-width:0;border-bottom:1px solid #e5eaed;padding:8px 9px}.campaign-agent-select{grid-column:1 / -1;border-top:0;padding:8px 10px}.campaign-agent-select select{max-width:none}.campaign-list-toolbar{align-items:flex-start}.campaign-visible-count{display:none}.campaign-filter-tabs{gap:18px}.campaign-client-row{grid-template-columns:1fr;padding:12px;contain-intrinsic-size:330px}.campaign-person-cell,.campaign-status-cell,.campaign-activity-cell,.campaign-action-cell{grid-column:1}.campaign-status-cell{align-items:flex-start}.campaign-status-meta{align-items:flex-start;flex-direction:row;gap:10px}.campaign-activity-cell{padding-top:8px}.campaign-action-cell{align-items:stretch}.campaign-primary-action{width:100%}.campaign-attempt-actions,.campaign-record-actions{justify-content:flex-start;gap:12px}.campaign-load-more-row{flex-direction:column;gap:7px}.campaign-load-more-row button{width:100%;min-height:44px}.outreach-dialog-backdrop{padding:9px}.outreach-dialog{padding:14px;max-height:calc(100dvh - 18px)}.outreach-followup-row{grid-template-columns:1fr}.outreach-dialog-actions .campaign-work-button{flex:1}.outreach-dialog-actions .campaign-quiet-button{flex:1}}
      `}</style>
    </>
  )
}
