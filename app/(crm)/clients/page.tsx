...
type AgentProfile = { id: string; full_name: string; role: string }
...
export default async function ClientsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const selectedAgent = canFilterByAgent && agents.some((agent) => agent.id === requestedAgent) ? requestedAgent : ''
  const healthCompanies = Array.from(new Set((healthPlanRows || []).map((row: { company_name: string | null }) => (row.company_name || '').trim()).filter(Boolean))).sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }))
  const selectedHealthCompany = healthCompanies.includes(requestedHealthCompany) ? requestedHealthCompany : ''
  ...
  const totalCountAgentId = canFilterByAgent ? selectedAgent || null : userId
  const { data: countRows, error: countError } = await supabase.rpc('crm_client_counts', { p_agent_id: totalCountAgentId })
  const countRow = Array.isArray(countRows) ? countRows[0] : null
  const totalClientCount = Number(countRow?.total_clients || 0)
  const totalMedicareCount = Number(countRow?.medicare_clients || 0)
  const totalNonMedicareCount = Number(countRow?.non_medicare_clients || 0)
  const totalCountError = countError?.message || ''
  ...
  if (shouldSearch) {
    ...
    if (selectedHealthCompany && !showAll) {
    ...
    if (showAll || !selectedHealthCompany || (healthClientIds && healthClientIds.length > 0)) {
      let query = supabase
        .from('clients')
        .select('id, assigned_agent_id, first_name, last_name, date_of_birth, phone, county, state, is_medicare, is_life, is_retirement, created_at', { count: 'exact' })
      ...
      if (sort === 'first_name') query = query.order('first_name', { ascending: true, nullsFirst: false }).order('last_name', { ascending: true, nullsFirst: false })
      else if (sort === 'county') query = query.order('county', { ascending: true, nullsFirst: false }).order('last_name', { ascending: true, nullsFirst: false }).order('first_name', { ascending: true, nullsFirst: false })
      else if (sort === 'created_desc') query = query.order('created_at', { ascending: false, nullsFirst: false }).order('last_name', { ascending: true, nullsFirst: false }).order('first_name', { ascending: true, nullsFirst: false })
      else if (sort === 'created_asc') query = query.order('created_at', { ascending: true, nullsFirst: false }).order('last_name', { ascending: true, nullsFirst: false }).order('first_name', { ascending: true, nullsFirst: false })
      else query = query.order('last_name', { ascending: true, nullsFirst: false }).order('first_name', { ascending: true, nullsFirst: false })

      if (!showAll && q) {
        const searchTerms = q.replace(/[,%()]/g, ' ').split(/\s+/).map((term) => term.trim()).filter(Boolean).slice(0, 8)
        for (const term of searchTerms) query = query.or(`first_name.ilike.%${term}%,last_name.ilike.%${term}%,phone.ilike.%${term}%,email.ilike.%${term}%`)
      }
      ...
      if (!showAll && age65plus) query = query.lte('date_of_birth', getAgeCutoffDate(65))
  ...
  const paginationBase: Record<string, string> = showAll ? {
    show_all: '1',
    ...
    ...(product ? { product } : {}),
    ...(turn65 ? { turn65: '1' } : {}),
    ...(age65plus ? { age65plus: '1' } : {}),
    ...(selectedAgent ? { agent: selectedAgent } : {}),
    ...(selectedHealthCompany ? { health_company: selectedHealthCompany } : {}),
    ...(sort !== 'last_name' ? { sort } : {})
  }
    ...
    <>
      ...
      <form className="toolbar" action="/clients" method="get">
        ...
        <select className="select" name="product" defaultValue={product} style={{ width: 180 }}>
          <option value="">All products</option><option value="life">Life</option><option value="medicare">Medicare</option><option value="retirement">Retirement</option><option value="life_medicare">Life + Medicare</option><option value="non_life">Non-Life</option><option value="non_medicare">Non-Medicare</option><option value="non_life_non_medicare">Non-Life + Non-Medicare</option>
        </select>
        <select className="select" name="sort" defaultValue={sort} style={{ width: 190 }} aria-label="Sort clients">
          <option value="last_name">Last name A-Z</option><option value="first_name">First name A-Z</option><option value="county">County A-Z</option><option value="created_desc">Newest added first</option><option value="created_asc">Oldest added first</option>
        </select>
      ...
      {!totalCountError ? (
        <>
          <div className="clients-stat-grid">
            <div className="card clients-stat-card"><div className="clients-stat-label">Total clients</div><div className="clients-stat-value">{totalClientCount}</div><div style={{ marginTop: 6 }}>{canFilterByAgent ? selectedAgent ? `${agentNames[selectedAgent] || 'This agent'} has ${totalClientCount} client${totalClientCount === 1 ? '' : 's'} in total.` : `Your agency has ${totalClientCount} client${totalClientCount === 1 ? '' : 's'} in total.` : `You have ${totalClientCount} client${totalClientCount === 1 ? '' : 's'} in total.`}</div></div>
            <div className="card clients-stat-card medicare"><div className="clients-stat-label">Medicare clients</div><div className="clients-stat-value">{totalMedicareCount}</div><div style={{ marginTop: 6 }}>Marked as Medicare.</div></div>
            <div className="card clients-stat-card non-medicare"><div className="clients-stat-label">Non-Medicare clients</div><div className="clients-stat-value">{totalNonMedicareCount}</div><div style={{ marginTop: 6 }}>Not marked as Medicare.</div></div>
          </div>
          <div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
            <Link prefetch={false} className="btn btn-secondary" href={`/clients?${new URLSearchParams({ show_all: '1', ...(selectedAgent ? { agent: selectedAgent } : {}), ...(sort !== 'last_name' ? { sort } : {}) }).toString()}`}>Show All</Link>
            {shouldSearch ? <span className="subtle" style={{ alignSelf: 'center' }}>Results are paged 50 at a time for consistent speed.</span> : null}
          </div>
        </>
      ) : null}

      {!shouldSearch ? (
        <section className="card"><div className="empty"><strong>No clients are loaded by default.</strong><br />Search above, choose a filter, choose a sort order and press Search, or use the T-65 or 65+ button.</div></section>
      ) : (
        <ClientsResults
          ...
          pagination={{ page, pageSize, total: filteredTotal, baseParams: paginationBase }}
        />
      )}
    </>
  )
