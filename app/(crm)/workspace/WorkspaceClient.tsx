...
export default function WorkspaceClient({ viewerId, viewerName, isManager, agents }: {
  ...
  const eventOwnerClients = useMemo(
    () => clients.filter((client) => client.assigned_agent_id === eventDraft.assigned_agent_id),
    [clients, eventDraft.assigned_agent_id]
  )
  const clientChoices = useMemo(() => {
    const search = clientSearch.trim().toLowerCase()
    const filtered = search
      ? eventOwnerClients.filter((client) => `${clientName(client)} ${client.phone || ''}`.toLowerCase().includes(search))
      : eventOwnerClients
  ...
  async function saveEvent() {
    ...
    try {
      ...
      setEventOpen(false)
      setDayOpenDate(eventDraft.event_date)
  ...
  async function saveReschedule() {
    ...
    try {
      const response = await fetch(`/api/workspace/events/${rescheduleEvent.id}`, {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'reschedule', note: rescheduleNote })
      })
      const result = await response.json().catch(() => ({}))
  ...
  return (
    <>
      <div className="workspace-heading">
        <div><h1>Workspace</h1><p className="subtle">Quick leads, follow-up work, appointments, and activities.</p></div>
      ...
      {isManager ? (
        <div className="workspace-owner-bar">
          ...
          {agents.map((agent) => (
            <button key={agent.id} type="button" className={`workspace-owner-filter ${agentTone(agent.full_name)}${ownerFilter === agent.id ? ' active' : ''}`} onClick={() => setOwnerFilter(agent.id)}>{agent.full_name.split(' ')[0]}</button>
          ))}
        </div>
      ) : null}
        ...
        <section className="workspace-panel">
          <div className="workspace-panel-head calendar-head">
            <div>
              ...
              <p className="subtle" style={{ margin: '7px 0 0' }}>Tap any date to open that day’s appointments and activities.</p>
            </div>
                  ...
                  <div className={`workspace-calendar-day${outside ? ' outside' : ''}${isToday ? ' today' : ''}`} key={key} role="button" tabIndex={0} onClick={() => setDayOpenDate(key)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setDayOpenDate(key) }}>
                    ...
                    <div className="workspace-day-events">
                      {dayItems.slice(0, 4).map((event) => {
                        ...
                        return (
                          <div key={event.id} className={`workspace-event ${agentTone(agent?.full_name || '')} ${event.status}`}>
                            <span>{formatTime(event.start_time)}{event.status === 'completed' ? ' · ✓' : event.status === 'needs_reschedule' ? ' · ↻' : ''}</span>
                            <strong>{event.title}</strong>
                            <small>{linked ? clientName(linked) : event.event_type === 'appointment' ? 'Appointment' : 'Activity'}</small>
                          </div>
                        )
                      })}
        ...
        <div className="workspace-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setDayOpenDate(null) }}>
          <section className="workspace-modal workspace-day-modal" role="dialog" aria-modal="true" aria-label={`Calendar for ${dayOpenDate}`}>
            <div className="workspace-modal-head">
              <div><h2>{formatDayHeading(dayOpenDate)}</h2><p className="subtle" style={{ margin: '5px 0 0' }}>{dayEvents.length} {dayEvents.length === 1 ? 'item' : 'items'}</p></div>
            ...
            {!dayEvents.length ? <div className="empty workspace-day-empty">Nothing scheduled for this day.</div> : null}
            <div className="workspace-day-list">
              {dayEvents.map((event) => {
                ...
                return (
                  <article className={`workspace-day-item ${tone} ${event.status}`} key={event.id}>
                    <div className="workspace-day-item-head">
                      <div>
                        <div className="workspace-day-item-pills">
                          ...
                          <span className="workspace-type-pill">{event.event_type === 'appointment' ? 'Appointment' : 'Activity'}</span>
                          {event.status === 'completed' ? <span className="workspace-status-pill completed">✓ Complete</span> : null}
                          {event.status === 'needs_reschedule' ? <span className="workspace-status-pill reschedule">Needs Reschedule</span> : null}
                        </div>
                        <h3>{event.title}</h3>
                    ...
                    {linked ? (
                      <div className="workspace-linked-client"><span>Client</span><strong>{clientName(linked)}</strong>{linked.phone ? <small>{linked.phone}</small> : null}</div>
                    ) : null}
                    {event.notes ? <div className="workspace-event-notes"><strong>Notes</strong><p>{event.notes}</p></div> : null}
                    {event.reschedule_note ? <div className="workspace-reschedule-note"><strong>Reschedule note</strong><p>{event.reschedule_note}</p></div> : null}
                    <div className="workspace-day-item-actions">
                      ...
                      {event.status !== 'completed' ? <button type="button" className="btn btn-secondary btn-small workspace-complete-btn" disabled={busy} onClick={() => void completeEvent(event)}>✓ COMPLETE</button> : null}
                      {event.status !== 'completed' ? <button type="button" className="btn btn-secondary btn-small" disabled={busy} onClick={() => openReschedule(event)}>RESCHEDULE</button> : null}
        ...
        <div className="workspace-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setEventOpen(false) }}>
          <section className="workspace-modal" role="dialog" aria-modal="true" aria-label={eventDraft.id ? 'Edit calendar item' : 'Add calendar item'}>
            ...
            <div className="workspace-modal-actions">
              {eventDraft.id ? <button type="button" className="btn btn-secondary" disabled={busy} onClick={() => { const existing = events.find((event) => event.id === eventDraft.id); if (existing) void deleteEvent(existing) }}>DELETE</button> : null}
